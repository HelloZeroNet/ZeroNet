    ...
    def actionBigfileUpload(self):
        ...
        if len(piecemap_info["sha512_pieces"]) == 1:  # Small file, don't split
            ...
            with site.storage.open(upload_info["piecemap"], "wb") as f:
                msgpack.pack({file_name: piecemap_info}, f)
...
class ContentManagerPlugin(object):
    ...
    def hashFile(self, dir_inner_path, file_relative_path, optional=False):
        ...
        if not hash or not piecemap_relative_path:  # Not in content.json yet
            ...
            with self.site.storage.open(inner_path, "rb") as f:
                merkle_root, piece_size, piecemap_info = self.hashBigfile(f, file_size)
            if not hash:
                hash = merkle_root
    ...
    def getPiecemap(self, inner_path):
        ...
        with self.site.storage.open(piecemap_inner_path) as f:
            piecemap = msgpack.unpack(f)[helper.getFilename(inner_path)]
