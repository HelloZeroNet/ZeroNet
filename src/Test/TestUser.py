...
class TestUser:
    ...
    def testNewSite(self, user):
        address, address_index, site_data = user.getNewSiteData("Bitcoin")  # Create a new random site
        assert Crypt.hdPrivatekey("Bitcoin", user.master_seed, address_index) == site_data["privatekey"]
    ...
    def testAuthAddress(self, user):
        ...
        assert Crypt.privatekeyToAddress(auth_privatekey) == auth_address
    ...
    def testCert(self, user):
        ...
        auth_privatekey = user.getAuthPrivatekey("1EU1tbG9oC1A8jz2ouVwGZyQ5asrNsE4Vr")
        assert Crypt.privatekeyToAddress(auth_privatekey) == cert_auth_address
