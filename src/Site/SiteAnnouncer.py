    ...
    def announceTrackerUdp(self, tracker_address, mode="start", num_want=10):
        ...
        if not back:
            ...
            if "peers" in back["response"]:
                peers.append(back["response"]["peers"])
            if "peers6" in back["response"]:
                peers.append(back["response"]["peers6"])
        else:
        ...
        try:
            ...
            if "peers6" in response_dict:
                ...
                for peer_offset in xrange(peer_count):
                    ...
                    ipv6_addr = hex(addr1)[2:] + ":" + hex(addr2)[2:] + ":" + hex(addr3)[2:] + ":" + hex(addr4)[2:] + ":" + hex(addr5)[2:] + ":" + hex(addr6)[2:] + ":" +hex(addr7)[2:] + ":" + hex(addr8)[2:]
                    peers.append({"addr": ipv6_addr, "port": port})
